...
impl Scene {
    ...
    /// Iterates over all root [`Transform`]s declared in this scene.
    ///
    ...
    fn root_game_objects<'a>(
        ...
        iter::from_fn(move || {
            let [first, _, third]: [Address; 3] = match scene_manager.pointer_size {
    ...
    /// Tries to find the specified root [`Transform`] in this scene.
